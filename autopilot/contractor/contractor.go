...
func (c *Contractor) performContractMaintenance(ctx *mCtx, w Worker) (bool, error) {
	...
	if err != nil {
		c.logger.Error("failed to fetch satellite config")
	}
	if cfg.Enabled {
		...
		if err != nil {
			c.logger.Error("failed to fetch renter settings")
		}
		if rs.AutoRenewContracts {
			satellite.StaticSatellite.RequestContracts(sctx)
		}
		if rs.AutoRepairFiles {
			satellite.StaticSatellite.RequestSlabs(sctx, ctx.ContractSet())
		}
		if rs.BackupFileMetadata {
			satellite.StaticSatellite.RequestMetadata(ctx, ctx.ContractSet())
		}
	}
...
LOOP:
	for h := 0; missing > 0 && h < len(selected); h++ {
		...
		var proceed bool
		if cfg.Enabled {
			config := ctx.ContractsConfig()
			formedContract, err = satellite.StaticSatellite.FormContract(ctx, host.PublicKey, ctx.EndHeight(), config.Storage, config.Upload, config.Download)
			proceed = true
		} else {
...
func (c *Contractor) runContractRenewals(ctx *mCtx, w Worker, toRenew []contractInfo, budget *types.Currency, limit int) (renewals []renewal, toKeep []api.ContractMetadata) {
	...
	for i = 0; i < len(toRenew); i++ {
		...
		if cfg.Enabled {
			config := ctx.ContractsConfig()
			renewed, err = satellite.StaticSatellite.RenewContract(ctx, toRenew[i].contract.ID, ctx.EndHeight(), config.Storage, config.Upload, config.Download)
			proceed = true
		} else {
...
func (c *Contractor) runContractRefreshes(ctx *mCtx, w Worker, toRefresh []contractInfo, budget *types.Currency) (refreshed []renewal, _ error) {
	...
	for _, ci := range toRefresh {
		...
		if cfg.Enabled {
			config := ctx.ContractsConfig()
			renewed, err = satellite.StaticSatellite.RenewContract(ctx, ci.contract.ID, ci.contract.EndHeight(), config.Storage, config.Upload, config.Download)
			proceed = true
		} else {
